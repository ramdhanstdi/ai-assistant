...
100% offline, tanpa cloud. Bisa diakselerasi di Intel Arc (xpu) bila Intel Extension
for PyTorch (IPEX) terpasang; jika tidak, otomatis jalan di CPU.
...
def pick_device(pref: str) -> str:
    ...
    try:
        import torch
        # torch.xpu tersedia bila Intel Extension for PyTorch (IPEX) terpasang.
...
class MMSTTSManager:
    def __init__(self, config_path="config.yaml"):
        ...
        self.model.eval()
        print(f"🔊 MMS-TTS siap (sample rate {self.sample_rate} Hz).")
